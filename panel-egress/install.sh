...
# Verify egress backend before touching the panel.
TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT
...
# Backup only panel integration. Networking is deliberately untouched.
echo
echo "===== BACKUP ====="
...
# Fetch exact upstream release source.
echo
echo "===== FETCH PANEL 1.0.14 SOURCE ====="
rm -rf "$WORK"
...
# Apply maintained custom sources from tyxe_pool.
echo
echo "===== PATCH SOURCE ====="
python3 "$SELF_DIR/patch.py" "$PANEL_SRC" "$SELF_DIR"

# Build frontend + static Go binary using upstream Dockerfile.
echo
echo "===== BUILD ====="
case "$(uname -m)" in
...
# Privileged bridge: panel stays unprivileged; only validated egress actions use sudo.
echo
echo "===== INSTALL BRIDGE ====="
...
echo "Rollback complete. Egress manager/AWG/node-agents were not changed."
EOF
chmod 700 /root/rollback-mtproxyl-panel-egress.sh

# Atomic-ish binary swap after successful build/bridge validation.
echo
...
echo "Bridge status:"
sudo -u mtproxyl-panel sudo -n "$BRIDGE" status | python3 - <<'PY'
import json, sys
...
echo "upstream Panel binary is blocked in this custom build so"
echo "the /egress integration cannot be overwritten accidentally."
echo "============================================================"
