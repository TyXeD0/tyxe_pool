...
[[ -x "$PROVISIONER" ]] || fail "Не установлен SSH provisioner. Сначала запусти egress-v1/install-provisioner.sh"
[[ -f "$SELF_DIR/job-runner.py" ]] || fail "Не найден job-runner.py"
id mtproxyl-panel >/dev/null 2>&1 || fail "Нет пользователя mtproxyl-panel"
systemctl is-active --quiet mtproxyl-egressd.service || fail "Dynamic Egress daemon не active."

/usr/local/libexec/mtproxyl-egress-registry validate
python3 -m py_compile "$SELF_DIR/job-runner.py"
...
echo
echo "============================================================"
echo " MTProxyL Panel Dynamic Egress UI egress3"
echo "============================================================"
...
print("Telemt NAT:", (d.get("telemt") or {}).get("nat_ip"))
print("Nodes:", len(nodes))
for n in sorted(nodes, key=lambda x:(int(x.get("priority",999999)), str(x.get("name","")))):
    print(f"  {n.get('priority'):>3} {n.get('name')} [{n.get('id')}] enabled={n.get('enabled')} health={n.get('health')} agent={(n.get('agent') or {}).get('reachable')}")
PY
"$PROVISIONER" --version
...
echo
echo "===== INSTALL BRIDGE + BACKGROUND JOBS ====="
install -d -m 755 /usr/local/libexec
install -o root -g root -m 755 "$SELF_DIR/job-runner.py" "$JOB_RUNNER"
...
restore_panel(){
    ...
    if [[ -f "$BACKUP/mtproxyl-egress-panel-bridge" ]]; then
        install -o root -g root -m 755 "$BACKUP/mtproxyl-egress-panel-bridge" "$BRIDGE"
    else
        rm -f "$BRIDGE"
    fi
    if [[ -f "$BACKUP/mtproxyl-egress-panel-job" ]]; then
        install -o root -g root -m 755 "$BACKUP/mtproxyl-egress-panel-job" "$JOB_RUNNER"
    else
        rm -f "$JOB_RUNNER"
    fi
    if [[ -f "$BACKUP/mtproxyl-panel-egress.sudoers" ]]; then
        install -o root -g root -m 440 "$BACKUP/mtproxyl-panel-egress.sudoers" "$SUDOERS"
    else
        rm -f "$SUDOERS"
    fi
...
install -o root -g root -m 755 "\$BACKUP/mtproxyl-panel" "\$PANEL_BIN"
if [[ -f "\$BACKUP/mtproxyl-egress-panel-bridge" ]]; then install -o root -g root -m 755 "\$BACKUP/mtproxyl-egress-panel-bridge" "\$BRIDGE"; else rm -f "\$BRIDGE"; fi
if [[ -f "\$BACKUP/mtproxyl-egress-panel-job" ]]; then install -o root -g root -m 755 "\$BACKUP/mtproxyl-egress-panel-job" "\$JOB_RUNNER"; else rm -f "\$JOB_RUNNER"; fi
if [[ -f "\$BACKUP/mtproxyl-panel-egress.sudoers" ]]; then install -o root -g root -m 440 "\$BACKUP/mtproxyl-panel-egress.sudoers" "\$SUDOERS"; else rm -f "\$SUDOERS"; fi
systemctl daemon-reload
...
echo "Rollback complete. Dynamic Egress networking and provisioner were not changed."
EOF
...
printf 'Egress daemon:  '; systemctl is-active mtproxyl-egressd.service
printf 'Provisioner:    '; "$PROVISIONER" --version
printf 'Panel jobs:     '; "$JOB_RUNNER" --version
...
echo "  add / remove / switch / test / rename / enable / disable / priority"
echo "  add/remove run as background jobs; SSH credentials are passed through stdin/tmpfs only"
echo "============================================================"
