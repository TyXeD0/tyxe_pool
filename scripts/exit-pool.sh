...
haproxy_edit(){
  local mode=$1 id=$2 ip=${3:-} tmp old
  tmp=$(mktemp /etc/haproxy/.tyxe-pool.XXXXXX)
  old=$(mktemp /etc/haproxy/.tyxe-pool-old.XXXXXX)
  cp -a "$HAPROXY" "$old"
  python3 - "$HAPROXY" "$tmp" "$mode" "$id" "$ip" <<'PY'
from pathlib import Path
import re,sys
src,dst,mode,node,ip=sys.argv[1:6]
...
if mode=='add':
    ...
    body.append(f'    server {node} {ip}:443 check inter 5s rise 2 fall 3 send-proxy-v2 backup')
elif mode=='remove':
    new=[]; found=False
    for l in body:
        m=rx.match(l)
...
clone_config(){
  local source=$1 target=$2 target_cfg=$3 target_ip=$4 src_cfg tmp
...
set -Eeuo pipefail
python3 - '$target_cfg' '$target_ip' '$PROXY_DOMAIN' <<'PY'
from pathlib import Path
import json,re,sys
p=Path(sys.argv[1]); ip=sys.argv[2]; host=sys.argv[3]; lines=p.read_text().splitlines()
def patch(sec,vals):
    global lines
...
patch('server',{'port':'443','listen_addr_ipv4':json.dumps(ip),'proxy_protocol':'true'})
...
ss -ltnpH 'sport = :443' | grep -F '$target_ip:443' >/dev/null
EOF
}
...
add(){
  ...
  local thost=$EXIT_HOST tip=$EXIT_IP teip=$ENTER_IP tif=$LOCAL_IF tport=$SSH_PORT tkey=$SSH_KEY
  ...
  echo "Source: $source"; echo "AWG: $teip -> $tip ($tif)"
  ...
  if ! clone_config "$source" "$id" "$tcfg" "$tip"; then ssh_for "$thost" "$tport" "$tkey"; rx "install -m 600 '$original' '$tcfg'; systemctl restart telemt" || true; return 1; fi
  timeout 5 bash -c "</dev/tcp/$tip/443" || { red 'Target TCP/443 недоступен; восстанавливаю target Telemt.'; ssh_for "$thost" "$tport" "$tkey"; rx "install -m 600 '$original' '$tcfg'; systemctl restart telemt" || true; return 1; }
  haproxy_edit add "$id" "$tip" || { ssh_for "$thost" "$tport" "$tkey"; rx "install -m 600 '$original' '$tcfg'; systemctl restart telemt" || true; return 1; }
  umask 077
  {
    printf 'NODE_ID=%q\n' "$id"; printf 'EXIT_HOST=%q\n' "$thost"; printf 'EXIT_IP=%q\n' "$tip"; printf 'ENTER_IP=%q\n' "$teip"; printf 'LOCAL_IF=%q\n' "$tif"
    printf 'SSH_PORT=%q\n' "$tport"; printf 'SSH_KEY=%q\n' "$tkey"; printf 'TELEMT_CONFIG=%q\n' "$tcfg"; printf 'ORIGINAL_TELEMT_BACKUP=%q\n' "$original"; printf 'SOURCE_NODE_ID=%q\n' "$source"
...
sync_node(){
  ...
  . "$POOL_DIR/$id.state"
  local thost=$EXIT_HOST tip=$EXIT_IP tport=${SSH_PORT:-22} tkey=${SSH_KEY:-} tcfg=$TELEMT_CONFIG
  source=${source:-$(primary_id || true)}; [[ -n $source && $source != "$id" ]] || { red 'Укажите отдельную source-ноду.'; return 1; }
  ssh_for "$thost" "$tport" "$tkey"; local bak="/var/lib/tyxe-pool-persistent/exit-pool-backups/$id/telemt.before-sync.$(date +%Y%m%d-%H%M%S)"; rx "cp -a '$tcfg' '$bak'"
  yesno "Синхронизировать users/secrets $source -> $id?" || return 0
  clone_config "$source" "$id" "$tcfg" "$tip" || { ssh_for "$thost" "$tport" "$tkey"; rx "install -m 600 '$bak' '$tcfg'; systemctl restart telemt" || true; return 1; }
...
status(){
  ...
  printf 'FakeTLS: %s\nPublic:  %s:443\n\n' "$FAKE_SNI" "$PROXY_DOMAIN"
  local line id addr ip role
  while IFS= read -r line; do
    [[ -n $line ]] || continue; id=$(awk '{print $2}' <<<"$line"); addr=$(awk '{print $3}' <<<"$line"); ip=${addr%:*}; role=PRIMARY; grep -qw backup <<<"$line" && role=BACKUP
    printf '%-18s %-8s %-16s TCP443=' "$id" "$role" "$ip"
    timeout 3 bash -c "</dev/tcp/$ip/443" 2>/dev/null && echo OPEN || echo DOWN
  done < <(servers)
}
